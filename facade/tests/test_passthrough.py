...
class TestBuildConvertOptionsPassthrough:
    ...
    def test_a_field_sent_twice_becomes_a_list(self):
        ...
        options = build_convert_options(_form([("custom_thing", "a"), ("custom_thing", "b")]))
    ...
    def test_an_arbitrary_field_the_facade_has_never_heard_of_still_passes_through(self):
        ...
        options = build_convert_options(_form([("some_future_docling_option", "value")]))
...
class TestHttpLayerPassthrough:
    def test_a_representative_sample_of_options_reaches_docling_serve_untouched(
        ...
        mock_docling_client.post.return_value = fake_response(
            200, json={"task_id": "opts-task", "task_type": "convert", "task_status": "pending"}
        )
    ...
    def test_options_are_not_required_defaults_still_work(self, client, mock_docling_client):
        ...
        mock_docling_client.post.return_value = fake_response(
            200, json={"task_id": "bare-task", "task_type": "convert", "task_status": "pending"}
        )
