...
- **`MODULE.bazel`** plus each component's own `BUILD.bazel` — reproducible Bazel/rules_oci builds for all three images, pinned to their upstream bases by digest. `Containerfile`s (root and `facade/`) remain as plain-`docker build` fallbacks for anyone without Bazel set up.
